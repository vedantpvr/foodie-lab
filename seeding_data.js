...
// ----------------------
async function main() {
  // 1. Clear existing data (Optional: good for clean tests)
  // await clearCollection("recipes"); 
  // await clearCollection("interactions");

  await insertPuranPoli();
  await insertSyntheticRecipes();
  await insertUsers();
  await insertInteractions(); // <--- This function has the fix
...
async function insertPuranPoli() {
  ...
  // Explicit ID used for boosting later
  const recipeRef = db.collection("recipes").doc("recipe_puran_poli");
  await recipeRef.set(puranPoli);
...
async function insertSyntheticRecipes() {
  const sampleRecipes = [
    "Masala Dosa", "Paneer Butter Masala", "Veg Pulao", "Gulab Jamun", "Kheer",
    "Aloo Paratha", "Poha", "Misal Pav", "Vada Pav", "Sabudana Khichdi",
    "Shrikhand", "Dal Tadka", "Jeera Rice", "Palak Paneer", "Upma",
    "Idli Sambhar", "Chicken Biryani", "Fish Curry", "Egg Curry", "Matar Paneer",
  ];
...
// ----------------------
// 4. INTERACTIONS (MODIFIED FOR COLD START FIX)
// ----------------------
async function insertInteractions() {
  const recipesSnap = await db.collection("recipes").get();
  const recipeIds = [];
  ...
  // --- CONFIG FOR THE FIX ---
  const PURAN_POLI_ID = "recipe_puran_poli";
  const BOOST_PERCENTAGE = 0.20; // 20% of traffic guaranteed for Puran Poli
  // --------------------------
  ...
  console.log(`🔄 Generating ${TOTAL_INTERACTIONS} interactions with Puran Poli Boost...`);

  for (let i = 0; i < TOTAL_INTERACTIONS; i++) {
    const docRef = db.collection("interactions").doc(`int_${i}`);
    
    // === LOGIC CHANGE START ===
    // Simulating a "New Arrival" algorithm where Puran Poli gets 20% exposure
    let selectedRecipeId;
    if (Math.random() < BOOST_PERCENTAGE) {
        selectedRecipeId = PURAN_POLI_ID;
    } else {
        selectedRecipeId = sample(recipeIds);
    }
    // === LOGIC CHANGE END ===

    const type = sample(interactionTypes);
    const rating = type === "rating" ? sample([null, 3, 4, 5]) : null;
  ...
  console.log("✅ Interactions inserted (Boost logic applied).");
}
